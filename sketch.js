...
function draw() {
 background("black");
  ...
  zombie.collide(invisible_ground); 
    ...
    spawngc();
    spawnObstacles();
    for(var i=0; i<gcGroup.length;i++){
    if(girl.isTouching(gcGroup.get(i))){
      gcGroup.get(i).destroy();
      score=score+1;
      }
    }
...
function spawngc(){
  if(frameCount%40===0){
    gc = createSprite(600,300,20,20);
    gc.y = Math.round(random(280,340));
    gc.addImage(gcImg);
    gc.scale=0.2;
    gc.velocityX=-6;
    gc.lifeTime=150;
